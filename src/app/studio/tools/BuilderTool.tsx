...
const REMIX_MODES: {
...
}[] = [
  {
    ...
    label: "Reskin",
    icon: Palette,
    desc: "Keep composition, change everything else",
  },
  {
    id: "style",
    label: "Extract Style",
    icon: Paintbrush,
    desc: "Pull the artistic style from reference",
  },
  {
    id: "composition",
    label: "Extract Layout",
    icon: Layout,
    desc: "Keep structure, change content",
  },
  {
    id: "mood",
    label: "Extract Mood",
    icon: Flame,
    desc: "Capture the atmosphere and feeling",
  },
];
...
export default function BuilderTool() {
  ...
  const [selectedOutput, setSelectedOutput] = useState<ForgeOutput | null>(
    null,
  );
  ...
  const [activePanel, setActivePanel] = useState<"input" | "forge" | "output">(
    "input",
  );
  ...
  const addLog = useCallback((level: ForgeLog["level"], message: string) => {
    ...
    setLogs((prev) =>
      [{ id: `log_${Date.now()}`, time, level, message }, ...prev].slice(
        0,
        100,
      ),
    );
  ...
  const buildRemixPrompt = useCallback(
    (basePrompt: string, mode: RemixMode, hasRef: boolean): string => {
      const modePrefix: Record<RemixMode, string> = {
        reskin:
          "Reimagine this scene with a completely new look and feel, different colors, textures, and style, but keep the same basic composition and layout",
        style:
          "Apply the artistic style and visual aesthetic from the reference to this new scene",
        composition:
          "Use the same spatial arrangement, layout, and compositional structure as the reference, but with entirely new subject matter",
  ...
  const handleForge = useCallback(async () => {
    ...
    if (coinBalance !== null && coinBalance < totalCost) {
      setError(
        `Not enough LiTBit Coins. Need ${totalCost}, have ${coinBalance}.`,
      );
      return;
    }
    ...
    addLog(
      "info",
      `⚡ FORGE STARTED — ${batchSize} variant(s) via ${currentProvider.label}`,
    );
    ...
    addLog(
      "info",
      `Mode: ${remixMode} · Aspect: ${aspectRatio} · Provider: ${currentProvider.label}`,
    );
    ...
    for (let i = 0; i < batchSize; i++) {
      ...
      try {
        ...
        addLog(
          "success",
          `[${i + 1}/${batchSize}] ✓ Variant forged · ${data.free ? "FREE" : data.cost + " 🪙"}`,
        );
    ...
    addLog(
      "info",
      `⚡ FORGE COMPLETE — ${newOutputs.length}/${batchSize} variants forged`,
    );
  }, [
    prompt,
    negativePrompt,
    ...
    >
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div
            className="hidden sm:flex items-center gap-1.5 px-2 py-0.5 rounded text-[9px] font-bold"
            style={{
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            style={{
              backgroundColor: showLogs ? T.accentColor + "15" : "transparent",
              color: showLogs ? T.accentColor : T.textMuted,
              border: `1px solid ${showLogs ? T.accentColor + "30" : T.borderColor + "20"}`,
            }}
          >
            <Terminal size={9} /> LOG
          </button>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div
          ...
          style={{
            borderRight: `1px solid ${T.borderColor}12`,
            ...
            >
              ...
              {referenceImage ? (
                <div
                  ...
                  style={{ border: `1px solid ${T.borderColor}30` }}
                >
                ...
                <div className="flex flex-wrap gap-1">
                  {STYLE_PRESETS.map((style) => (
                    <button
                      ...
                      onClick={() =>
                        setPrompt((prev) =>
                          prev ? `${prev}, ${style.toLowerCase()}` : style,
                        )
                      }
                      disabled={isWorking}
        ...
        >
          {/* Forge Controls Bar */}
          <div
            className="shrink-0 flex items-center justify-between px-4 py-2 flex-wrap gap-2"
            style={{
              borderBottom: `1px solid ${T.borderColor}10`,
              backgroundColor: T.boxBg + "20",
            }}
          >
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <select
                value={providerId}
                onChange={(e) =>
                  setProviderId(e.target.value as MediaProviderId)
                }
                disabled={isWorking}
                    ...
                    style={{
                      backgroundColor:
                        aspectRatio === opt.value
                          ...
                          : "transparent",
                      color:
                        aspectRatio === opt.value ? T.accentColor : T.textMuted,
              ...
              <div className="flex items-center gap-0.5">
                {([1, 2, 4] as const).map((n) => (
                  <button
                    ...
                    style={{
                      backgroundColor:
                        batchSize === n ? T.accentColor + "20" : "transparent",
              ...
              <div className="h-full flex flex-col">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <button
                      onClick={() => {
                        navigator.clipboard.writeText(selectedOutput.url);
                      }}
                      className="flex items-center gap-1 px-2 py-1 text-[9px] font-bold rounded transition-all hover:scale-105"
                      style={{
                ...
                <div
                  className="mt-2 flex items-center gap-3 text-[9px]"
                  style={{ color: T.textMuted }}
                >
                  <span>
                    Mode:{" "}
                    <b style={{ color: T.accentColor }}>
                      {selectedOutput.mode}
                    </b>
                  </span>
                  <span>
                    Provider: <b>{selectedOutput.provider}</b>
                  </span>
                  <span>
                    Seed: <b>{selectedOutput.seed}</b>
                  </span>
                  <span>
                    {new Date(selectedOutput.timestamp).toLocaleTimeString()}
                  </span>
                </div>
              </div>
            ) : (
              <div className="h-full flex flex-col items-center justify-center gap-4 opacity-40">
                ...
                <div className="text-center">
                  ...
                  <p className="text-[10px] mt-1">
                    Upload a reference, set your mode, write a prompt, and hit
                    Forge.
                  </p>
                  <p
                  ...
                  >
                    Design, test, and launch — all from one battle-ready studio.
                  </p>
                </div>
              </div>
            )}
        ...
        <div
          ...
          style={{
            borderLeft: `1px solid ${T.borderColor}12`,
            backgroundColor: T.boxBg + "20",
          }}
        >
          {/* Console Header */}
          ...
          {showLogs && (
            <div
              className="shrink-0 h-48 flex flex-col"
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-2 space-y-0.5 font-mono">
                {logs.length === 0 ? (
                  <div className="text-[9px] opacity-30 text-center mt-4">
                    No log entries
                  </div>
                ) : (
                  logs.map((log) => (
      ...
      <div
        ...
        style={{
          borderTop: `1px solid ${T.borderColor}10`,
          backgroundColor: T.boxBg + "30",
        }}
      >
        <div className="flex items-center gap-3">
          ...
          <span
            ...
            style={{ color: T.textMuted }}
          >
            {referenceImage ? "✓ Reference loaded" : "No reference"} ·{" "}
            {remixMode} · {aspectRatio} · {currentProvider.label}
          </span>
        </div>
        <div className="flex items-center gap-2">
          {error && (
            <span
              ...
              style={{ color: error.includes("✓") ? "#3fb950" : "#f85149" }}
            >
          ...
          >
            <span
              ...
              style={{
                backgroundColor: status === "forging" ? "#d29922" : "#3fb950",
                boxShadow: `0 0 4px ${status === "forging" ? "#d29922" : "#3fb950"}`,
              }}
            />
