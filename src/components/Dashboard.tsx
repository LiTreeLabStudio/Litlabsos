  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (
          g.status === "fulfilled" &&
          (Array.isArray(g.value?.items) || Array.isArray(g.value))
        ) {
        ...
        if (
          f.status === "fulfilled" &&
          (Array.isArray(f.value?.posts) || Array.isArray(f.value))
        ) {
      ...
      : [
          {
            id: "1",
            author: "Pixel Forge",
            content: "Generated new landscape concepts for the arctic zone.",
            time: "5m ago",
            likes: 8,
            replies: 2,
          },
          {
            ...
            author: "Director",
            content:
              "All agent systems are operating within optimal parameters.",
            time: "20m ago",
            likes: 14,
            replies: 0,
          },
    ...
    <div className="w-full max-w-[1600px] mx-auto px-4 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-1">
          ...
          <p className="text-sm opacity-50">
            Your workspace is synchronized across all nodes.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((stat) => (
          <div
            key={stat.label}
            className="glass-card p-6 flex items-center justify-between group hover:border-indigo-500/30 transition-all"
          >
            <div className="space-y-1">
            ...
            <div
              className="p-3 rounded-2xl bg-white/5 group-hover:scale-110 transition-transform"
              style={{ color: stat.color }}
            >
              <stat.icon size={24} />
            </div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {tools.map((tool) => (
                <Link
                  key={tool.label}
                  href="/studio"
                  className="p-3 rounded-xl bg-white/5 border border-transparent hover:border-indigo-500/20 transition-all text-center"
                >
                  <tool.icon
                    size={18}
        ...
        <div className="xl:col-span-6 space-y-8">
          <div className="glass-card overflow-hidden">
            <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between">
              ...
              <span className="text-[10px] font-bold text-indigo-400 bg-indigo-400/10 px-2 py-1 rounded-lg">
                Realtime
              </span>
            </div>
          ...
          <div className="glass-card p-6 space-y-6">
            ...
            <div className="space-y-3">
              {activeAgents.map((agent) => (
                <div
                  key={agent.id}
                  className="p-3 rounded-2xl bg-white/5 border border-transparent hover:border-white/10 transition-all flex items-center gap-3"
                >
                  <div
                  ...
                  <div className="flex flex-col items-end gap-1">
                    <span className="text-[8px] font-black uppercase px-2 py-0.5 rounded-full border border-white/10 opacity-60">
                      {agent.status}
                    </span>
                  </div>
                </div>
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={ach.label} className="flex items-center gap-3">
                  <div
                    className={`w-2.5 h-2.5 rounded-full ${ach.done ? "bg-indigo-500 shadow-[0_0_8px_rgba(99,102,241,0.6)]" : "bg-white/10"}`}
                  />
                  <span
                    className={`text-xs font-bold ${ach.done ? "" : "opacity-30"}`}
                  >
                    {ach.label}
                  </span>
                  {ach.done && (
...
<h3 className="text-xl font-black leading-tight">
              ...
              <Link
                href="/hub"
                className="flex items-center justify-center gap-2 w-full py-3 bg-white text-indigo-600 rounded-xl font-black text-xs uppercase tracking-widest hover:scale-105 transition-transform active:scale-95"
              >
                Launch Hub
              </Link>
            </div>
